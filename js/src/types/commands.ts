import { Result, Enable, PlayState, RepeatMode, Mute, Shuffle, Error, SystemError, FirmwareVersion } from "./constants"
import { CommaSeparatedList, PlayerId, PlayerInfo, PlayingMedia, PlayingStationOptions, QueueId, QueueItem, QuickselectId, QuickselectInfo } from "./payloads"
...
export namespace Response {
  type FailedResponse<Command extends Command.System | Command.Player> = {
    heos: {
      command: Command
      result: Result.Fail,
  ...
  type SuccessfulResponse<Command extends Command.System | Command.Player, SuccessMessage extends string> = {
    heos: {
      command: Command
  ...
  type FailableResponse<Command extends Command.System | Command.Player, SuccessMessage extends string> = SuccessfulResponse<Command, SuccessMessage> | FailedResponse<Command>

  type FailableResponseWithPayload<Command extends Command.System | Command.Player, SuccessMessage extends string, Payload extends Object | Array<any>> =
    (SuccessfulResponse<Command, SuccessMessage> & { payload: Payload }) | FailedResponse<Command>

  type FailableResponseWithPayloadAndOptions<Command extends Command.System | Command.Player, SuccessMessage extends string, Payload extends Object | Array<any>, Options extends Array<any>> =
  ...
  export namespace Player {
    ...
    export type GetNowPlayingMedia = FailableResponseWithPayload<Command.Player.GetNowPlayingMedia, `pid=${PlayerId}`, Extract<PlayingMedia, {type: 'song'}>>
    ...
    export type GetMute = FailableResponse<Command.Player.GetMute, `${PID}&state=${Mute}`>
    export type SetMute = FailableResponse<Command.Player.SetMute, `${PID}&state=${Mute}`>
    export type ToggleMute = FailableResponse<Command.Player.SetMute, PID>
    export type GetPlayMode = FailableResponse<Command.Player.GetPlayMode, `${PID}&repeat=${RepeatMode}&shuffle=${Shuffle}`>
    export type SetPlayMode = FailableResponse<Command.Player.SetPlayMode, `${PID}&repeat=${RepeatMode}&shuffle=${Shuffle}`>
    export type GetQueue = FailableResponseWithPayload<Command.Player.GetQueue, `pid=${PlayerId}` | `${PID}&range=${number},${number}`, Array<QueueItem>>
    export type PlayQueue = FailableResponse<Command.Player.PlayQueue, `${PID}&qid=${QueueId}`>
    export type RemoveFromQueue = FailableResponse<Command.Player.RemoveFromQueue, `${PID}&qid=${CommaSeparatedList}`> // No typing possible for comma separated list
    ...
    export type MoveQueueItem = FailableResponse<Command.Player.MoveQueueItem, `${PID}&sqid=${CommaSeparatedList}&dqid=${QueueId}`>
